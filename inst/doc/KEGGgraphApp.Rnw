...
%\VignetteDepends{graph, XML, Rgraphviz, RBGL, org.Hs.eg.db, KEGG.db, SPIA, hgu133plus2.db}
...
logcol <- cols[incol]; logcol[logfcs==0] <- "darkgrey"; logcol[!nodes(g) %in% allKID] <- "yellow"
...
\caption{Up-regulated genes (seven, in red) and their neighborhood genes in colorectal
  cancer pathway, the color follows the legend in figure \ref{fig:spiaVis}.}\label{fig:spiaSub}
\end{figure}
