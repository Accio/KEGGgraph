...
%\VignetteDepends{graph, XML, Rgraphviz, RBGL, org.Hs.eg.db, tinytex}
...
<<install_tex, eval=FALSE, echo=FALSE>>=
library(tinytex)
...
name and identifier is use \Robject{KEGGPATHNAME2ID} environment that was originally provided in
\Rpackage{KEGG.db}, which is now provided by \Rpackage{KEGGgraph} because \Rpackage{KEGG.db} is deprecated. For example, the following codes retrieve p53
...
<<bcc, echo=TRUE, eval=FALSE, results=verbatim>>=
library(RBGL)
...
\begin{figure}[!hp]
\begin{center}
<<bccplot, eval=FALSE, echo=TRUE, fig=TRUE>>=
...
\label{convertId}
<<orgHuman, eval=FALSE, echo=TRUE,  fig=TRUE>>=
if(require(org.Hs.eg.db)) {
